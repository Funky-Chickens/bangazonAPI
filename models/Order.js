...
let formatOrder = (order) => {
    ...
    order.forEach(orderItem => {
        formattedOrder.products.push({"product_id": orderItem.product_id, 
            "name": orderItem.product_name, 
            "price": orderItem.price, 
            "quantity": orderItem.quantity_avail, 
...
let deleteNoProdOrders = (id) => {
    return new Promise( (resolve, reject)=>{
        db.all(`SELECT * 
            FROM productOrders 
            WHERE line_item_id = ${id}`, (err, lineId)=> {
            if (err) return reject(err);
            if (lineId[0]) {
                db.all(`SELECT * 
                    FROM productOrders 
                    WHERE order_id = ${lineId[0].order_id}
                `, (err, orderData)=>{
    ...
    putOrder:(id, orderObj) => { //need whole orderObj, but use the passed in ID from the req.params in order to access that number even after the object has been deleted from the DB
        return new Promise( (resolve, reject) => {
            db.run(`UPDATE orders 
                SET order_date = "${orderObj.order_date}", 
                payment_type = ${orderObj.payment_type}, 
                buyer_id = ${orderObj.buyer_id}
                WHERE order_id = ${id}`
                , (err, order) => {
                if (err) return reject(err);
                resolve(order);
    ...
    postProdOrderObj:(prodOrderObj) => { //this prodOrderObj is the req.body passed from the ordersCtrl
        return new Promise((resolve, reject) => {
            db.run(`INSERT INTO productOrders 
                VALUES (${prodOrderObj.order_id}, 
                ${prodOrderObj.product_id}, 
                null)`, (err, prodOrder) => {
                if (err) return reject(err);
                resolve(prodOrder);
            });
